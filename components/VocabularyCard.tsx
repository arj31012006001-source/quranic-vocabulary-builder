...
const VocabularyCard: React.FC<VocabularyCardProps> = ({ word, isLearned, onToggleLearned, isBookmarked, onToggleBookmark, note, onUpdateNote, onRootSearch, rootCounts, onDerivativeSearch }) => {
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl ${isLearned ? 'opacity-70' : ''}`}>
      <div className="p-6 md:p-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4">
          <div className="flex-grow">
            <div className="flex items-center gap-2 mb-1">
              <h2 dir="rtl" className="arabic-text text-4xl md:text-5xl font-bold text-emerald-600 dark:text-emerald-400">
                {word.arabic}
              </h2>
            ...
            <p className="text-xl text-slate-500 dark:text-slate-400 font-mono tracking-wide" aria-label="Phonetic pronunciation">
              /{word.transliteration}/
            </p>
          </div>
          <div className="mt-2 md:mt-0 text-right flex-shrink-0">
        ...
        <div className="mb-6 space-y-2">
            <div className="flex items-center space-x-3">
                ...
                <span className="text-xs font-semibold bg-sky-100 text-sky-800 px-2.5 py-1 rounded-full dark:bg-sky-900 dark:text-sky-300">
                    {word.partOfSpeech}
                </span>
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border-l-4 border-emerald-500">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  {rootCounts[word.root] > 1 ? (
                     <Tooltip content="Find related words from this root">
                       <button
                         onClick={() => onRootSearch(word.root!)}
            ...
            <div className="flex flex-wrap gap-2">
              {word.derivatives.map(derivative => (
                <Tooltip key={derivative.arabic} content={`Search for "${derivative.arabic}"`}>
                  <button
                    onClick={() => onDerivativeSearch(derivative.arabic)}
